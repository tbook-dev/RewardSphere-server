        ...
        return if (r.isSuccess) {
            ...
            if (tws.isEmpty()) {
                ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, cookie.toString())
                        .body(SimpleResponse.failed("no twitter binding"))
            } else {
                ResponseEntity.ok().header(HttpHeaders.SET_COOKIE, cookie.toString())
                        .body(SimpleResponse.success(""))
            }
        } else {
            ResponseEntity.status(HttpStatus.FORBIDDEN).body(SimpleResponse.failed("login failed"))
        }
    }
